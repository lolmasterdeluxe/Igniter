...
namespace IG
{
    public class RotateTowardsTargetState : State
    {
        ...
        public override State Tick(EnemyManager enemyManager, EnemyStats enemyStats, EnemyAnimatorManager enemyAnimatorManager)
        {
            ...
            {
                enemyAnimatorManager.PlayTargetAnimationWithRootRotation("Turn Behind Left", true);
                return combatStanceState;
            }
            else if (viewableAngle <= -101 && viewableAngle >= -180 && !enemyManager.isInteracting)
            {
                enemyAnimatorManager.PlayTargetAnimationWithRootRotation("Turn Behind Right", true);
                return combatStanceState;
            }
            ...
            else if (viewableAngle <= 100 && viewableAngle >= 45 && !enemyManager.isInteracting)
            {
